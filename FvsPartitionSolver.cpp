...
set<int> fvsPartitionSolver(map<int, multiset<int>> g, set<int> v1, set<int> v2, int K, bool found = 0) {
    set<int> solution;
    if(K <= 0 && isForest(g) == 0) {
        found = 0;
        return solution;
    ...
    for(int w : v1) {
        ...
        for(int i = 0; i < l - 1; i ++) {
            for(int j = i + 1; j < l; j ++) {
                if(isReachable(gv2, neighV2[i], neighV2[j])) {
                    ...
                    set<int> sol = fvsPartitionSolver(newg, newv1, v2, K-1, 0);
                    if(found == 1) {
                        sol.emplace(w);
                    }
                    return sol;
        ...
        set<int> newv2 = v2;
        v2.emplace(w);
        set<int> sol2 = fvsPartitionSolver(g, newv1, newv2, K, 0);
        if(found == 1) {
            return sol2;
        }
        return solution;
    ...
    for(auto w : v1) {
        if((int)(gv1[w].size()) <= 1) {
            if(originalDegree[w] <= 1) {
                ...
                set<int> newv1 = v1;
                newv1.erase(w);
                set<int> sol = fvsPartitionSolver(newg, newv1, v2, K);
                return sol;
            }
            else {
                ...
                set<int> sol = fvsPartitionSolver(newg, v1, v2, K);
            }
        }
    }
